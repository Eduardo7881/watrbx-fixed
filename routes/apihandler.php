...
$router->post('/api/v1/verify-captcha', function(){
    ...
    $localBypass = isset($_ENV["DEV_BYPASS_CAPTCHA"]) &&
                   $_ENV["DEV_BYPASS_CAPTCHA"] === "true";

    if(!$localBypass && !isset($_POST["captcha"])){
        die(create_error("No captcha was provided!"));
    }

    if(!$localBypass && !$auth::verifycaptcha($_POST["captcha"])){
        die(create_error("The captcha provided is invalid!"));
    }
...
$router->post('/api/v1/signup', function() {
    ...
    $tokenQuery = $db->table("captchaverified")
    ...
    // Token is valid only while its expiration timestamp is in the future.
    if($tokenRecord->time <= time()){
        $db->table("captchaverified")
            ->where("token", $captchaToken)
            ->delete();
    ...
    if($result["code"] == 200){
        // Captcha sessions are one-time use.
        $db->table("captchaverified")
            ->where("token", $captchaToken)
            ->delete();
