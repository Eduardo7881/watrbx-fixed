...
<script>
    ...
    const canregister = <?=json_encode($_ENV["CAN_REGISTER"] === "true")?>;
    const localCaptchaBypass = <?=json_encode(
        isset($_ENV["DEV_BYPASS_CAPTCHA"]) &&
        $_ENV["DEV_BYPASS_CAPTCHA"] === "true"
    )?>;
...
<script>
    ...
    function verifyCaptcha() {
        ...
        xhr.open("POST", "/api/v1/verify-captcha", true);
        ...
        if(localCaptchaBypass){
            // /api/v1/verify-captcha creates the local development token.
            xhr.send("captcha=local-development-bypass");
            return;
        }

        var captchaElement =
            document.getElementsByName("cf-turnstile-response")[0];
        ...
        xhr.send(
            "captcha=" + encodeURIComponent(captchaElement.value)
        );
    }
    ...
    if(localCaptchaBypass && canregister){
        // No Cloudflare Turnstile is required on local development.
        verifyCaptcha();
    }
